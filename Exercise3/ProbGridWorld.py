    ...
    def move(self, direction):
        if direction == 'u':
                ...
                if (self.position - 1) % self.size != self.size-1:
                    self.position -= 1
        return self.position, -1 if not self.over() else 5
...
def startValue(game, valueIterator, maxMoves = 200):
    game.reset()
    gameover = False
    totalReward = 0
    moves = 0
...
def startPolicy(game, policyIterator, maxMoves = 200):
    game.reset()
    gameover = False
    totalReward = 0
    moves = 0
...
class ValueIterator:
    ...
    def decideMoveWithUpdate(self, state):
        ...
        for a in self.actions:
            totalReward = 0
            for actions in allowedActions:
                self.game.position = state
                newPos, reward = self.game.move(actions)
                probability = 0.91 if a == actions else 0.03
                reward += self.gamma * self.values[newPos]
                reward *= probability
                totalReward += reward
            maxReward = max(maxReward, totalReward)
            actionReward[a] = totalReward
    ...
    def evaluatePolicy(self):
        ...
        while not converged:
            ...
            for state in self.states:
                ...
                total = 0
                probability = 1 / len(self.policy[state])
                for a in self.policy[state]:
                    self.game.position = state
                    state_, reward = self.game.move(a)
                    total += probability * (reward + self.gamma * self.values[state_])
        ...
        for state in self.states:
            ...
            for a in self.actions:
                ...
                for actions in allowedActions:
                    probability = 0.91 if a == actions else 0.03
                    self.game.position = state
                    state_, reward = self.game.move(actions)
                    reward += self.gamma * self.values[state_]
                    totalReward += probability*reward
                maxReward = max(maxReward, totalReward)
                actionRewards[a] = totalReward

            self.policy[state] = [
                action for action,
                reward in actionRewards.items() if reward == maxReward
                ]
            if np.all(oldActions != self.policy[state]):
                stable = False
        return stable
...
if __name__ == "__main__":
    ...
    # Arrange
    size = 100 # Length and width of the grid
